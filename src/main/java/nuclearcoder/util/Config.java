...
public class Config {
	...
	static
	{
		...
		configFile = new File(UtilConstants.CONFIG_FILENAME);
		...
		if (!configFile.exists()) // copy the default file if it doesn't exist
		{
			...
			try
			{
				in = Config.class.getResourceAsStream("/" + UtilConstants.CONFIG_FILENAME);
	 ...
	 * Returns the property with the specified key in this 
	 * configuration. If the key is not found in the list, 
 	 * the method returns the second argument and the property is set.
	 * @param key the property key
